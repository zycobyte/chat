...
function onSend(message) {
    ...
    */
    ...
    if ((message.value.trim() === "" && toSend == 0) || message.value.length > 1000) {
...
function joinChat(newChat){
	...
	// checkValidity();
		...
		var dataToSend = JSONData({"action": "join", "server": newChatID});
...
function joinDm(user){
    ...
    var dataToSend = JSONData({"action": "join", "server": "dm", "user":formatMessage(user)});
